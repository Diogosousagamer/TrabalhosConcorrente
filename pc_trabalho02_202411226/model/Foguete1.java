...
* Nome.............: Foguete1
* Funcao...........: Esta classe gerencia o movimento e as demais propriedades do foguete 1
...
public class Foguete1 extends Thread {
   ...
   * ***************************************************************
   * Metodo: Foguete1
   * Funcao: carrega uma nova instancia da Thread Foguete1
   * Parametros: 
  ...
  public Foguete1(int velocidade, ImageView imagemFoguete, String posicao, String sentido, String colisao) {
   ...
   * Funcao: define o angulo e posicao iniciais do foguete1
   ...
   * Funcao: retorna o foguete1 a sua posicao inicial
  ...
  private void redefinirPosicao() {
    ...
    // Inicio do bloco Platform.runlater
    Platform.runLater(() -> {
      // O foguete1 e reposicionado no ponto inicial da trilha
  ...
  public void run() {
    ...
    while (!Thread.currentThread().isInterrupted()) {
      ...
      if (TelaTrilhaEspacialController.isPlaying) {
        ...
        if (sentido.equals("DA ESQUERDA PARA DIREITA")) {
          moverEsquerdaParaDireita(); // o foguete1 vai se mover da esquerda para direita
        } 
        else { // Caso contrario
   ...
   * Funcao: percorre o foguete1 da esquerda para direita apenas uma vez antes de voltar ao ponto inicial
   * Parametros: nenhum parametro foi especificado para esta funcao
   * Retorno: void
   ****************************************************************/

  private void moverEsquerdaParaDireita() {
    // O foguete1 e rotacionado constantemente e percorre do ponto -40 ate chegar no ponto 700 no eixo X
    // A posicao no eixo Y e definida de acordo com a posicao do foguete1 em relacao a trilha
    // A rotacao padrao e 0, e ela e alterada de acordo com a posicao do foguete1 em relacao a trilha
   ...
   * Funcao: percorre o foguete1 da direita para esquerda apenas uma vez antes de voltar ao ponto inicial
   * Parametros: nenhum parametro foi especificado para esta funcao
   * Retorno: void
   ****************************************************************/

  private void moverDireitaParaEsquerda() {
    // O foguete1 e rotacionado constantemente e percorre do ponto 700 ate chegar no ponto -40 no eixo X
    // A posicao no eixo Y e definida de acordo com a posicao do foguete1 em relacao a trilha
    // A rotacao padrao e -180, e ela e alterada de acordo com a posicao do foguete1 em relacao a trilha
   ...
   * Funcao: o foguete1 se movimenta apenas pelo eixo X
   * Parametros: int destinoX (ponto de chegada do foguete1)
   * Retorno: void
   ****************************************************************/

  private void seguirParaFrente(int destinoX) {
    // Inicio do bloco while
    // Enquanto o foguete1 estiver em movimento 
    ...
    while (!Thread.currentThread().isInterrupted() && (int) Math.round(posX) != destinoX) {
      ...
      // Caso isPlaying estiver falso, o foguete1 fica pausado antes de atualizar a posicao
      pausar();
   ...
   * Funcao: o foguete1 se movimenta pelas diagonais, modificando
   * tanto a posicaoX quanto a posicaoY
   * Parametros: int destinoX (ponto de chegada do foguete1 no eixo X)
   * int destinoY (ponto de chegada do foguete1 no eixo Y)
   * Retorno: void
   ****************************************************************/
   ...
   * Funcao: o foguete1 vai acessar a primeira regiao critica da trajetoria
  ...
  private void acessarTrilhaCompartilhada1(String colisao, int destinoX) {
    ...
    switch (colisao) {
      case "VARIAVEL DE TRAVAMENTO": // Caso variavel de travamento
       ...
       boolean primeiraVez = true;
        ...
        while (TelaTrilhaEspacialController.variavelTravamento1 != 0) {
          // Inicio do bloco if
          // Se e a primeira vez que o foguete1 acessa a regiao
          if (primeiraVez) {
            // Inicio do bloco try/catch
            try { 
        ...
        // A variavel de travamento e igual a 1 e o foguete1 se move pela regiao critica
        TelaTrilhaEspacialController.variavelTravamento1 = 1;
        seguirParaFrente(destinoX);

        break;

      case "ESTRITA ALTERNANCIA": // Caso for estrita alternancia
        // A booleana primeiraVez se torna verdadeira para evitar chamadas repetitivas
        primeiraVez = true;

        // Inicio do bloco while
        // Enquanto nao chegar a vez do foguete1 passar pela regiao critica
        while (TelaTrilhaEspacialController.vez1 != 0) {
          // Inicio do bloco if
          // Se e a primeira vez que o foguete1 acessa a regiao
          if (primeiraVez) {
            // Inicio do bloco try/catch
        ...
        // Ao chegar sua vez, o foguete1 segue pela regiao critica e ao sair
        // da a vez para o foguete2
        seguirParaFrente(destinoX);
        TelaTrilhaEspacialController.vez1 = 1;  
      ...
      case "SOLUCAO DE PETERSON": // Caso for solucao de Peterson
        ...
        // Inicio do bloco while
        // Enquanto a vez for dada ao foguete1 e o foguete2 ainda estiver passando pela regiao critica
        ...
        // Ao chegar sua vez, o foguete1 segue pela regiao critica
        seguirParaFrente(destinoX);
   ...
   * Funcao: o foguete1 vai acessar a segunda regiao critica da trajetoria
   * Parametros: String colisao (algoritmo de colisao selecionado)
   * int destinoX (ponto de chegada do foguete1)
   * Retorno: void
   ****************************************************************/
  ...
  private void acessarTrilhaCompartilhada2(String colisao, int destinoX) {
    ...
    switch (colisao) {
      case "VARIAVEL DE TRAVAMENTO": // Caso variavel de travamento
        ...
        // A variavel de travamento e igual a 1 e o foguete1 segue pela regiao critica
      ...
      case "ESTRITA ALTERNANCIA": // Caso estrita alternancia
        ...
        // Inicio do bloco while
        // Enquanto nao for a vez do foguete1 passar pela regiao
        while (TelaTrilhaEspacialController.vez2 != 0) {
          // Inicio do bloco if
          // Se e a primeira vez que o foguete1 acessa a regiao
          if (primeiraVez) {
            // Inicio do bloco try/catch
        ...
        // Ao chegar sua vez, o foguete1 passa pela regiao critica 
        // e da a vez para o foguete2
        seguirParaFrente(destinoX);
        TelaTrilhaEspacialController.vez2 = 1;  
      ...
      case "SOLUCAO DE PETERSON": // Caso solucao de Peterson
        ...
        // O foguete1 passa pela regiao critica
        seguirParaFrente(destinoX);
   ...
   * Funcao: o foguete1 sinaliza sua saida da primeira regiao critica da trajetoria
   * Parametros: String colisao (algoritmo de colisao selecionado)
   * Retorno: void
   ****************************************************************/
  ...
  public void sairTrilhaCompartilhada1(String colisao) {
    ...
    switch (colisao) {
      ...
      case "ESTRITA ALTERNANCIA": // Caso estrita alternancia
        // A vez e dada para o foguete2
        TelaTrilhaEspacialController.vez1 = 1;
        break;
      
      case "SOLUCAO DE PETERSON": // Caso solucao de Peterson
        // O processo do foguete1 e desativado
   ...
   * Funcao: o foguete1 sinaliza sua saida da segunda regiao critica da trajetoria
   * Parametros: String colisao (algoritmo de colisao selecionado)
   * Retorno: void
   ****************************************************************/
   ...
   * ***************************************************************
   * Metodo: pausar
   * Funcao: mantem o foguete1 pausado enquanto o movimento estiver interrompido
   * Parametros: nenhum parametro foi definido para esta funcao
   * Retorno: void
   ****************************************************************/
  ...
  private void pausar() {
    // Inicio do bloco while
    // Enquanto o movimento do foguete1 nao estiver ativo
   ...
   * Funcao: define o valor da velocidade do movimento do foguete1
   ...
   * ***************************************************************
   * Metodo: setVelocidade
   * Funcao: retorna o valor da velocidade do movimento do foguete1
   * Parametros: nenhum parametro foi especificado para esta funcao
   * Retorno: void
   ****************************************************************/
   ...
   * ***************************************************************
   * Metodo: setPosicao
   * Funcao: define o valor da posicao do foguete1 em relacao a trilha
   ...
   * ***************************************************************
   * Metodo: setSentido
   * Funcao: define o valor do sentido da trajetoria do foguete1
   ...
   * ***************************************************************
   * Metodo: getSentido
   * Funcao: retorna o valor do sentido da trajetoria do foguete1
   * Parametros: nenhum parametro foi especificado para esta funcao
   * Retorno: String
   ****************************************************************/
   ...
   * ***************************************************************
   * Metodo: setColisao
   * Funcao: define o algoritmo de colisao a ser operado no movimento do foguete1
   ...
   * ***************************************************************
   * Metodo: getColisao
   * Funcao: retorna o algoritmo de colisao a ser operado no movimento do foguete1
   * Parametros: nenhum parametro foi especificado para esta funcao
   * Retorno: void
   ****************************************************************/
