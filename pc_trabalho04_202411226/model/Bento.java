...
* Nome.............: Bento
* Funcao...........: esta classe gerencia o percurso de Bento pela trilha (PO5:SH)
...
public class Bento extends Thread {
  /* Atributos */
  ...
  // Sprites de Bento nos sentidos horizontal e vertical
  private static final Image horizontal = new Image(Bento.class.getResource("/assets/BentoCorrendo1.png").toExternalForm());
  private static final Image vertical = new Image(Bento.class.getResource("/assets/BentoCorrendo2.png").toExternalForm());
   ...
   * ***************************************************************
   * Metodo: Bento
   * Funcao: inicializa uma nova instancia da Thread Bento
   * Parametros: ImageView sprite - imagem do cachorro
   * int velocidade - velocidade do cachorro
   * Retorno: nenhum
   ****************************************************************/

  public Bento(ImageView sprite, int velocidade) {
  ...
  public void run() {
    // Define a posicao inicial de Bento
    definirPosicao(287, 15);

    // Inicio do bloco while
    // Rnquanto a Thread estiver ativa
    while (!Thread.currentThread().isInterrupted()) {
      // Inicio do bloco if
      if (TelaPrincipalController.bentoPlaying) { // Se o movimento de Bento estiver ativo
   ...
   * Funcao: realiza o primeiro percurso de Bento
   * Parametros: nenhum parametro foi definido para esta funcao
   * Retorno: void
   ****************************************************************/
  ...
  private void percorrerCaminho1() {
    ...
    try { // O metodo tenta realizar o seguinte percurso
      configurarImagem(vertical, 30, 51, 0, 0, 0, 1);
      seguirVertical(30);
   ...
   * Funcao: realiza o segundo percurso de Bento
   * Parametros: nenhum parametro foi definido para esta funcao
   * Retorno: void
   ****************************************************************/
  ...
  private void percorrerCaminho2() {
    ...
    try { // O metodo tenta realizar o seguinte percurso
      ...
      seguirVertical(224);
      configurarImagem(horizontal, 47, 51, -180, 0, 5, 1);
   ...
   * Funcao: realiza o terceiro percurso de Bento
   * Parametros: nenhum parametro foi definido para esta funcao
   * Retorno: void
   ****************************************************************/
  ...
  private void percorrerCaminho3() {
    ...
    try { // O metodo tenta realizar o seguinte percurso
      ...
      seguirParaFrente(90);
      configurarImagem(vertical, 30, 51, 180, 0, 0, 1);
      seguirVertical(130);
   ...
   * Funcao: realiza o quarto percurso de Bento
   * Parametros: nenhum parametro foi definido para esta funcao
   * Retorno: void
   ****************************************************************/
  ...
  private void percorrerCaminho4() {
    // Inicio do bloco try/catch
    try { // O metodo tenta realizar o seguinte percurso
      configurarImagem(horizontal, 47, 51, 0, 0, 0, 1);
   ...
   * double altura: altura da imagem
   * double largura: largura da imagem
  ...
  private void configurarImagem(Image imagem, double altura, double largura, double rotacao, double x, double y, double z) {
    // Inicio do bloco Platform.runLater
    Platform.runLater(() -> {
      // Troca a imagem
      sprite.setImage(imagem);

      // Troca a altura da imagem
      sprite.setFitHeight(altura);

      // Troca a largura da imagem
  ...
  private void seguirParaFrente(int destinoX) {
    ...
    while ((!Thread.currentThread().isInterrupted()) && ((int) Math.round(posX) != destinoX)) {
      ...
      // Realiza a pausa antes de atualizar a posicao caso Bento estiver pausado
      pausar();
  ...
  public void pausar() {
    // Inicio do bloco while
    // Enquanto Bento estiver pausado
    while (!TelaPrincipalController.bentoPlaying) { 
      // Inicio do bloco try/catch
   ...
   * Funcao: define o valor da velocidade do movimento de Bento
   ...
   * Funcao: retorna o valor da velocidade do movimento de Bento
   * Parametros: nenhum parametro foi definido para esta funcao
   * Retorno: void
   ****************************************************************/
